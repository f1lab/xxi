    ...
    <table class="table-condensed add-remove-chzn-for-relations add-remove-datetimepicker-for-relations relations-table">
        ...
        <?php if (!isset($onlyNew) and isset($form[$relationName])): ?>
            ...
            <?php if (count($form[$relationName])): ?>
                <?php foreach ($form[$relationName] as $i => $relation): ?>
                    <tr>
                        ...
                        <td align="center">
                            <input type="checkbox"
                                   name="<?php echo $form->getName() ?>[<?php echo $relationName ?>][<?php echo $i; ?>][delete_object]"
                                   id="<?php echo $form->getName() ?>_<?php echo $relationName ?>_[<?php echo $i;
                                   $i++; ?>]_delete_object">
                        </td>
                        <td>
                            <div style="color:red">
            ...
            <?php else: ?>
                <tr>
                    <td colspan="3">
                        <div class="alert alert-info"><?php echo isset($noRelationsMessage) ? $noRelationsMessage
        ...
        <?php if (isset($form['new_' . $relationName])): ?>
            ...
            <tr>
                <td>
                    <button type="button" class="btn ahAddRelation" rel="new_<?php echo $relationName ?>">
                        +<?php echo isset($addLabel) ? ' ' . $addLabel : '' ?></button>
