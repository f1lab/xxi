...
<ul class="pager pull-right"><?php
    ...
    <li <?php if ($version - 1 < 1):
        ...
        <a href="<?php echo ($cantclick1 ? '#' : '') . url_for('@order?id=' . $order->getId() . '&version=' . ($version
                    - 1))
        ?>">← версия <?php echo $version - 1 ?></a>
    </li>
    <li class="active">
        версия <?php echo $version ?> от <?php echo $order->getUpdator()->getUsername() ?>
        от <?php echo $order->getUpdatedAt() ?>
    </li>
    <li <?php if ($order->getAuditLog()->getMaxVersion($sf_data->getRaw('order')) == $version):
        $cantclick2 = true;
        echo ' class="disabled"';
    endif ?>>
        <a href="<?php echo ($cantclick2 ? '#' : '') . url_for('@order?id=' . $order->getId() . '&version=' . ($version
                    + 1))
        ?>">версия <?php echo $version + 1 ?>→</a>
    </li>
</ul>
...
<div class="btn-toolbar">
    ...
    if ( //TODO: replace this mess with some permission or move this check to model
        ...
        && in_array($order->getState(),
            ['calculating', 'prepress', 'prepress-working', 'prepress-done', 'work', 'working',])
        ...
        ?>
        <div class="btn-group">
            <a href="<?php echo url_for('@order-edit?id=' . $order->getId()) ?>"
    ...
    <?php if ($sf_user->hasCredential('can_print_orders')): ?>
        <div class="btn-group">
            <a href="<?php echo url_for('@order-print?id=' . $order->getId()) ?>" class="btn">Распечатать бланк заказа
                (цех)</a>
        </div>
    <?php endif ?>
    ...
    <?php if ($sf_user->hasCredential(["can-print-design-orders", "design-worker"], false)): //false is for OR switch ?>
        <div class="btn-group">
            <a href="<?php echo url_for('@order-print-design?id=' . $order->getId()) ?>" class="btn">Распечатать бланк
                заказа (дизайн/препресс)</a>
        </div>
    <?php endif ?>
    ...
    <?php if ($sf_user->hasGroup('manager') or $sf_user->hasGroup('buhgalter')): ?>
        <div class="btn-group">
            <a href="<?php echo url_for('@order-printaccount?id=' . $order->getId()) ?>" target="_blank" class="btn">Распечатать
                счет</a>
        </div>
    <?php endif ?>
    ...
    <?php if ($sf_user->hasGroup('buhgalter') || $sf_user->hasGroup('worker') and $order->getSubmitedAt()): ?>
        <div class="btn-group">
            <a href="<?php echo url_for('@order-printinvoice?id=' . $order->getId()) ?>" target="_blank" class="btn">Распечатать
                счет-фактуру</a>
            <a href="<?php echo url_for('@order-printwaybill?id=' . $order->getId()) ?>" target="_blank" class="btn">Распечатать
                ТТН</a>
        </div>
    <?php endif ?>
</div>
...
<?php if (true == ($comments = $order->getComments()) and count($comments)): ?>
    <section class="comments row"><?php foreach ($comments as $id => $comment): ?>
            <article class="comment<?php echo $comment->getReadAndMarkAsRead() ? '' : ' unread' ?> well span8"
                     id="comment-<?php echo $comment->getId() ?>">
        ...
        <?php endif */ ?>
                    <a href="#comment-<?php echo $comment->getId() ?>"
                       class="date muted pull-right"><?php echo $comment->getCreatedAt() ?></a>
                </div>
                <div class="content">
