...
<form action="<?php echo url_for("MaterialMovement/create"); ?>" method="post">
    ...
    <?php if ($type !== "utilization" or $sf_user->hasCredential("master")): ?>
        <fieldset>
            <legend>
                Список материалов
                <?php if ($type === "arrival"): ?>
                    <button class="btn btn-default" id="add-new-material-from-arrival-form">Добавить материал
                    </button><?php endif ?>
            </legend>
            <?php if ($type === "arrival"): ?>
                <div class="copy-me">
            ...
            <?php else: ?>
                <?php if (isset($balance)) foreach ($balance->getRawValue() as $material): ?>
                    <div class="control-group">
                        <label for="materials[<?php echo $material["id"]; ?>]"
                               class="control-label"><?php echo $material["name"]; ?></label>
                        <div class="controls">
                            <div class="input-append copy-number-inputs-max-to-value">
                                <input type="number" value="0" min="0" max="<?php echo $material["amount"]; ?>"
                                       step="0.0001" class="span2" name="materials[<?php echo $material["id"]; ?>]"
                                       id="materials[<?php echo $material["id"]; ?>]">
                                <button class="btn" type="button">Выбрать всё (<?php echo $material["amount"]; ?>)
                                </button>
                            </div>
                            <input type="hidden" name="materials_descriptions[<?php echo $material["id"]; ?>]"
                                   id="materials_descriptions[<?php echo $material["id"]; ?>]">
                        </div>
                    </div>
                <?php endforeach ?>
            <?php endif ?>
    ...
    <div class="form-actions">
        ...
        <a href="<?php echo url_for($type === "utilization" ? "plan/index"
            : "warehouse/index?id=" . $sf_request->getParameter("from")) ?>" class="btn">
            Назад к
